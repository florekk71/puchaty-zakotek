...
function renderVisits(){
  ...
  $('visitList').innerHTML=table(['Termin','Klient','Pies','Status','Kwota','Płatność','Akcje'],data.visits.filter(v=>(!start||v.visit_date.slice(0,10)>=start)&&(!end||v.visit_date.slice(0,10)<=end)).map(v=>row([
    esc(v.visit_date.slice(0,16)),esc(v.client),esc(v.dog),esc(status[v.status]||v.status),money(Math.round(Number(v.amount_total)*100)),v.status==='completed'?(v.paymentState.received?'Otrzymana':'Oczekuje'):'—',
...
function renderCalendar(){
 ...
 for(let i=0;i<days;i++){const d=new Date(start);d.setDate(d.getDate()+i);const iso=calendarIso(d);const visits=data.visits.filter(v=>v.visit_date.slice(0,10)===iso&&v.status!=='cancelled').sort((a,b)=>a.visit_date.localeCompare(b.visit_date));html.push('<section class="calendar-day"><h4>'+esc(d.toLocaleDateString('pl-PL',{weekday:'short',day:'numeric',month:'numeric'}))+'</h4>'+(canWrite()?'<button class="btn ghost" data-plan-date="'+iso+'">+ Wizyta</button>':'')+visits.map(v=>'<article class="calendar-event"><strong>'+esc(v.visit_date.slice(11,16))+' · '+esc(v.dog)+'</strong><div>'+esc(v.client)+'</div><p>'+esc(v.notes||'')+'</p><small>'+(v.status==='planned'?'Planowana':'Rozliczona')+'</small>'+(canWrite()&&v.status==='planned'?'<p><button class="btn ghost" data-edit-visit="'+Number(v.rowid)+'">Edytuj termin</button> <button class="btn soft" data-checkout="'+Number(v.rowid)+'">Rozlicz</button> <button class="btn ghost" data-cancel="'+Number(v.rowid)+'">Odwołaj wizytę</button></p>':'')+'</article>').join('')+(visits.length?'':'<p>Brak wizyt</p>')+'</section>');}
...
function clientForm(id=0,context={}){
 ...
 const html=selectField('clientKind','Rodzaj klienta',[['person','Osoba fizyczna bez NIP'],['company','Firma / osoba z NIP']],c.clientKind||(c.nip?'company':'person'))+field('name','Imię i nazwisko / nazwa',c.name,'text','required maxlength="128"')+field('phone','Telefon',c.phone,'tel','maxlength="20"')+field('email','E-mail',c.email,'email','maxlength="128"')+field('town','Miejscowość',c.town,'text','maxlength="128"')+field('address','Adres',c.address)+field('zip','Kod pocztowy',c.zip)+field('nip','NIP (puste dla osoby prywatnej)',c.nip)+field('alias','Skrócona nazwa',c.alias)+field('www','Strona www',c.www)+field('contact','Osoba kontaktowa',c.contact)+noteField('notes','Notatki',c.notes);
